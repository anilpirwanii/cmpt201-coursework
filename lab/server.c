...
The client is using Transfer Control Protocol (TCP) and connects to 127.0.0.1 on
port 8001 to send data to the server.
2. What data is the client sending to the server?
The cleint is sending 5 words: Hello, Apple, Car, Green, Dog.
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
It receives a pointer to `struct acceptor_args` and this struct contains the
'run' flag, list handle, and mutex.
2. How are received messages stored?
They are stored in a singly linked list and we can tell that "struct list_node"
code
3. What does `main()` do with the received messages?
It iterates through the linked list using "collect_all()" and prints/counts
them.
4. How are threads used in this sample code?
the run_acceptor accepts connections while the run_client handles readnig data
from each specific client. Explain the use of non-blocking sockets in this lab.
"fcntl(fd, F_SETFL, flags | O_NONBLOCK)" makes sockets non-blocking for the
listening socket and client sockets. Then it also allows threads to check for
data/connections without freezing which is critical for the shutdown logic aka
checking the `run` flag.
*/
